...
describe('CORE-13: Real-Time Chat & Communication', () => {
  ...
  it('should allow system messages from non-participants', () => {
    const channel = chatEngine.createChannel(['user1', 'user2']);
    const msg = chatEngine.sendMessage(channel.id, 'system', 'User joined', 'system');
  ...
  it('should throw when sending to a non-existent channel', () => {
    expect(() => {
      chatEngine.sendMessage('ch_nonexistent', 'user1', 'Hi');
  ...
  it('should support pagination with limit and offset', () => {
    ...
    for (let i = 1; i <= 5; i++) {
      chatEngine.sendMessage(channel.id, 'user1', `Message ${i}`);
    }

    const page1 = chatEngine.getMessages(channel.id, 2, 0);
    expect(page1).toHaveLength(2);
    expect(page1[0]!.content).toBe('Message 5');

    const page2 = chatEngine.getMessages(channel.id, 2, 2);
  ...
  it('should return empty array for channel with no messages', () => {
    const channel = chatEngine.createChannel(['user1', 'user2']);
    expect(chatEngine.getMessages(channel.id)).toHaveLength(0);
  });
  ...
  it('should only mark specified messages as read', () => {
    ...
    const msg1 = chatEngine.sendMessage(channel.id, 'user1', 'Message 1');
    chatEngine.sendMessage(channel.id, 'user1', 'Message 2');

    chatEngine.markAsRead(channel.id, [msg1.id]);

    const messages = chatEngine.getMessages(channel.id);
    expect(messages[0]!.status).toBe('sent');  // Message 2 still unread
    expect(messages[1]!.status).toBe('read');  // Message 1 marked read
  });

  it('should send image message type', () => {
    const channel = chatEngine.createChannel(['user1', 'user2']);
    const msg = chatEngine.sendMessage(channel.id, 'user1', 'https://img.url/photo.jpg', 'image');
